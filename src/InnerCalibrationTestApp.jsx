...
function VersionBanner() {
  ...
  return (
    <div className="border-b border-slate-200 bg-slate-50/90">
      <div className="flex w-full flex-wrap items-center justify-between gap-2 px-3 py-2 sm:px-4 lg:px-6">
        <div className="flex flex-wrap items-center gap-2 text-xs sm:text-sm">
          <span className="rounded-full border border-slate-200 bg-white px-3 py-1 font-semibold text-slate-950">
            版本 {APP_VERSION}
          </span>
          <span className="text-slate-600">
            {isWeChat ? "微信里如果看到旧页面，点右侧强制刷新最新版。" : "如果页面没更新，可以点右侧刷新最新版。"}
          </span>
        </div>
        <button
          onClick={forceRefreshToLatest}
          className="inline-flex items-center gap-2 rounded-full border border-slate-200 bg-white px-3 py-2 text-xs font-semibold text-slate-700 shadow-sm transition hover:border-slate-300 hover:bg-slate-100 sm:text-sm"
        >
          <Icon name="RotateCcw" className="h-4 w-4" />
          强制刷新最新版
        </button>
